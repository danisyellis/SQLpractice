...
SELECT country AS Big_and_NotGrowing FROM countries_by_population WHERE population > 20000000 AND population_change < 0;

SELECT rank, country, population,
    CASE
        WHEN density_per_sq_km > 80 THEN 'super-dense'
        WHEN density_per_sq_km > 70 THEN 'pretty crowded'
        WHEN density_per_sq_km > 60 THEN 'not too bad'
        ELSE 'so much space :-)'
        END as how_crowded,
density_per_sq_km
FROM countries_by_population GROUP BY rank;


SELECT AVG(population) AS Average_Population_Of_Top_100_Countries FROM countries_by_population;
